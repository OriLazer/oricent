<?php
	...
	if(isset($_POST['registerUsername'])){
		...
		if(!empty($registerFullName) && !empty($registerUsername) && !empty($registerRole) && !empty($registerPassword1) && !empty($registerPassword2)){
			...
			//Check if username is empty
			if($registerRole == ''){
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Please Select A Role.</div>';
				exit();
			}
			...
			if($usernameCheckingStatement->rowCount() > 0){
				...
				if($registerPassword1 !== $registerPassword2){
					...
					$insertUserSql = 'INSERT INTO user(fullName, username, password, role_id) VALUES(:fullName, :username, :password, :role_id)';
					$insertUserStatement = $conn->prepare($insertUserSql);
					$insertUserStatement->execute(['fullName' => $registerFullName, 'username' => $registerUsername, 'password' => $hashedPassword, 'role_id' => $registerRole]);
